...
type EnvoyFilterReconciler interface {
	ReconcileEnvoyFilter(obj *networking_istio_io_v1beta1.EnvoyFilter) (reconcile.Result, error)
}
...
type EnvoyFilterReconcilerFuncs struct {
	OnReconcileEnvoyFilter         func(obj *networking_istio_io_v1beta1.EnvoyFilter) (reconcile.Result, error)
	OnReconcileEnvoyFilterDeletion func(req reconcile.Request) error
}

func (f *EnvoyFilterReconcilerFuncs) ReconcileEnvoyFilter(obj *networking_istio_io_v1beta1.EnvoyFilter) (reconcile.Result, error) {
...
type EnvoyFilterFinalizer interface {
	...
	// finalize the object before it is deleted.
	// Watchers created with a finalizing handler will a
	FinalizeEnvoyFilter(obj *networking_istio_io_v1beta1.EnvoyFilter) error
}
...
func NewEnvoyFilterReconcileLoop(name string, mgr manager.Manager, options reconcile.Options) EnvoyFilterReconcileLoop {
	return &envoyFilterReconcileLoop{
		// empty cluster indicates this reconciler is built for the local cluster
		loop: reconcile.NewLoop(name, "", mgr, &networking_istio_io_v1beta1.EnvoyFilter{}, options),
	}
}
