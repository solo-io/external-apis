...
func makeGenericMutatingWebhookConfigurationSet(
	...
	genericSortFunc := func(toInsert, existing ezkube.ResourceId) bool {
		objToInsert, ok := toInsert.(client.Object)
		if !ok {
			objToInsert = &admissionregistration_k8s_io_v1.MutatingWebhookConfiguration{
				ObjectMeta: metav1.ObjectMeta{
					Name:      toInsert.GetName(),
					Namespace: toInsert.GetNamespace(),
				},
		...
		objExisting, ok := existing.(client.Object)
		if !ok {
			objExisting = &admissionregistration_k8s_io_v1.MutatingWebhookConfiguration{
				ObjectMeta: metav1.ObjectMeta{
					Name:      existing.GetName(),
					Namespace: existing.GetNamespace(),
				},
	...
	genericEqualityFunc := func(a, b ezkube.ResourceId) bool {
		objA, ok := a.(client.Object)
		if !ok {
			objA = &admissionregistration_k8s_io_v1.MutatingWebhookConfiguration{
				ObjectMeta: metav1.ObjectMeta{
					Name:      a.GetName(),
					Namespace: a.GetNamespace(),
				},
		...
		objB, ok := b.(client.Object)
		if !ok {
			objB = &admissionregistration_k8s_io_v1.MutatingWebhookConfiguration{
				ObjectMeta: metav1.ObjectMeta{
					Name:      b.GetName(),
					Namespace: b.GetNamespace(),
				},
