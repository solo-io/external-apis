...
func makeGenericTrafficSplitSet(
	...
	genericSortFunc := func(toInsert, existing ezkube.ResourceId) bool {
		...
		if !ok {
			objToInsert = &split_smi_spec_io_v1alpha1.TrafficSplit{
		...
		if !ok {
			objExisting = &split_smi_spec_io_v1alpha1.TrafficSplit{
				ObjectMeta: metav1.ObjectMeta{
	...
	genericEqualityFunc := func(a, b ezkube.ResourceId) bool {
		...
		if !ok {
			objA = &split_smi_spec_io_v1alpha1.TrafficSplit{
				ObjectMeta: metav1.ObjectMeta{
					Name:      a.GetName(),
		...
		if !ok {
			objB = &split_smi_spec_io_v1alpha1.TrafficSplit{
				ObjectMeta: metav1.ObjectMeta{
					Name:      b.GetName(),
